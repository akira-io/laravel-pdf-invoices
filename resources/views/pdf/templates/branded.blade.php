			...
			<div>
				...
				<p class="text-xs text-gray-600 leading-tight">
					...
					@if($invoice->seller->vatNumber)
						{{ $translator->__('vat_id') }}: {{ $invoice->seller->vatNumber }}<br>
					@endif
	...
	<div class="px-12 py-8">
		@if($invoice->issuedAt || $invoice->dueAt)
			<div class="flex gap-8 mb-6">
				@if($invoice->issuedAt)
					<div class="flex-1">
						<p class="text-xs uppercase font-semibold text-gray-500 mb-0">{{ $translator->__('invoice_date') }}</p>
						<p class="text-sm font-semibold text-gray-900">{{ $invoice->issuedAt->format('d M Y') }}</p>
					</div>
				@endif
				@if($invoice->dueAt)
					<div class="flex-1">
						<p class="text-xs uppercase font-semibold text-gray-500 mb-0">{{ $translator->__('due_date') }}</p>
						<p class="text-sm font-semibold text-gray-900">{{ $invoice->dueAt->format('d M Y') }}</p>
					</div>
				@endif
			</div>
		@endif
		<div class="grid grid-cols-2 gap-8 mb-6">
			<div>
				<p class="text-xs uppercase font-semibold text-gray-500 mb-2">{{ $translator->__('invoice_from') }}</p>
				<p class="font-bold text-sm text-gray-900">{{ $invoice->seller->name }}</p>
				<p class="text-xs text-gray-600">
					@if($invoice->seller->address)
						{{ $invoice->seller->address }}<br>
					@endif
					...
					@endforelse
					@if($invoice->seller->vatNumber)
						{{ $translator->__('vat_id') }}: {{ $invoice->seller->vatNumber }}
					@endif
				</p>
			</div>
			<div>
				<p class="text-xs uppercase font-semibold text-gray-500 mb-2">{{ $translator->__('invoice_to') }}</p>
				<p class="font-bold text-sm text-gray-900">{{ $invoice->buyer->name }}</p>
				<p class="text-xs text-gray-600">
					...
					@if($invoice->buyer->vatNumber)
						{{ $translator->__('vat_id') }}: {{ $invoice->buyer->vatNumber }}
					@endif
				</p>
			</div>
		</div>
		<table class="w-full mb-6 text-xs">
			...
			<tr>
				...
				<th class="px-4 py-2 text-right font-bold text-gray-900 uppercase">{{ $translator->__('qty') }}</th>
				<th class="px-4 py-2 text-right font-bold text-gray-900 uppercase">{{ $translator->__('tax') }}</th>
				<th class="px-4 py-2 text-right font-bold text-gray-900 uppercase">{{ $translator->__('amount') }}</th>
			</tr>
			</thead>
			<tbody>
